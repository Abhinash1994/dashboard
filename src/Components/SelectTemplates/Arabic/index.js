...
import ArabicPost from '../Arabic/arabicPost'
import TemplatePost from '../Arabic/templateArabic'
...
class Arabic extends Component {
  constructor(props) {
   ...
   handleLanguage(e) {
       ...
       axios.post('http://54.152.91.239/mission_dashboard_api.php?action=deletearticleinfo',{
          ...
          article_id:id,
               ...
               .then(res => {
                ...
                console.log(res)
                window.location="/post-submit/ar";
    ...
    componentWillMount(){
      this.getLanguage();
       axios.post('http://54.152.91.239/mission_dashboard_api.php?action=getArticle&mission_user_id=1&language_id=3')
               .then(res => {
                this.setState({
                  showdata: res.data,
                });
                console.log("arabic",res)
                // console.log("orbaci data",this.state.showdata)
              })
        }
         ...
         {this.state.showdata.map((p, i) => 
                <Grid item xs={3} >
                  <Paper className={classes.paper} style={{marginLeft:'22px'}}>
                      <ArabicPost {...p} key={i} id={p.id}  handleDelete={this.handleDelete}/>
                  </Paper>
...
export default withStyles(styles)(Arabic);
